...
# IDEA 4: DIGITAL TWIN SPATIAL MAP & IDEA 7: 4D SPATIAL KNOWLEDGE
CITY_ASSETS = {
    "infrastructure": {
        "roads": {"x": [0, 50, None, 25, 25], "y": [25, 25, None, 0, 50]},
        "buildings": [
            {"name": "HOME", "pos": [45, 5], "icon": "🏠", "desc": "Resident-8A"},
            {"name": "HOSPITAL", "pos": [5, 45], "icon": "🏥", "desc": "Emergency-Zone"},
            {"name": "SCHOOL", "pos": [35, 45], "icon": "🏫", "desc": "Primary-Edu"},
            {"name": "PARK", "pos": [10, 10], "icon": "🌳", "desc": "Green-Domain"}
        ]
    },
    "hazards": [{"name": "CONSTRUCTION", "pos": [20, 25], "icon": "🚧"}]
}
...
def create_world_view(drone_pos, trail_x, trail_y, ghost_x, ghost_y, show_hazards=False):
    ...
    # IDEA 15: NEURAL GHOST PATH (Comparing Adaptive vs Standard)
    if ghost_x:
        fig.add_trace(go.Scatter(x=ghost_x, y=ghost_y, mode='lines', 
                                 line=dict(color='#ff4b4b', width=1, dash='dot'), name="Standard Logic"))
    
    if trail_x:
        fig.add_trace(go.Scatter(x=trail_x, y=trail_y, mode='lines', 
                                 line=dict(color='#00FF41', width=3), name="HAWK AI Logic"))
    ...
    # IDEA 10: STRESS-TESTER (Dynamic Obstacle Injection)
    if show_hazards:
